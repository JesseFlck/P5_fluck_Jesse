    ...
    .then((product) => {
        ...
        for (let selectColor of product.colors) {
            document.querySelector(
                '#colors'
            ).innerHTML += `<option value="${selectColor}">${selectColor}</option>`;
        }
    });
...
addToCart.addEventListener('click', function (e) {
    ...
    if (basket.color == [] || basket.quantity <= 0 || basket.quantity >= 100) {
        alert(
            'Merci de renseigner une couleur et une quantité entre 0 et 100 !'
        );
    } else {
        ...
        if (
            ...
            for (element of storageCart) {
                if (element.id == basket.id && element.color == basket.color) {
                    ...
                    element.quantity =
                        parseInt(basket.quantity) + parseInt(element.quantity);
                }
            }
            if (productExists) {
